...
# Ye Windows aur Raspberry Pi dono par sahi path dhoond lega
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
DB_PATH = os.path.join(BASE_DIR, 'cubix_inventory.db')

# WINDOWS FIX: SQLAlchemy SQLite URI requires forward slashes (/)
...
# ... (Add/Restock/Load/Unload/Archive routes same as before) ...
...
def load_ams():
    ...
    if printer and fil:
        if data['loadType'] == 'new': fil.full_spools -= 1
        else: fil.half_spools -= 1
...
def unload_ams():
    data = request.json
    printer = Printer.query.get(data['printerId'])
    f_id = getattr(printer, f"slot{data['slotIdx']}")
    if f_id:
        fil = Filament.query.get(f_id)
